...
MODULE_PARM_DESC(manual_only,
		 "Keep DRM connectors disconnected and disable automatic scanout; allow only manual userspace JPEG injection and guarded probe logging");
...
static void t6_init_head_addressing(struct t6_device *t6, struct t6_head *head)
{
	...
	if (head->output_idx == 0) {
		...
		head->cmd_limit_addr = head->cmd_addr + (8 * 1024 * 1024);
	}
...
/* ──────────────────────────────────────────────────────────────────
 * Legacy URB pool helpers
 *
 * The current transport path uses synchronous bulk transfers, so probe and
 * disconnect no longer allocate or wait on this pool. Keep the helpers around
 * only until the older async path is removed completely.
 * ────────────────────────────────────────────────────────────────── */
...
struct urb *t6_get_urb(struct t6_device *t6)
{
	...
	wait_event_interruptible_timeout(t6->urbs.sleep,
		({
			unsigned long flags;
			spin_lock_irqsave(&t6->urbs.lock, flags);
...
static int t6_bulk_write(struct t6_device *t6, const void *src, size_t len)
{
	...
	chunk_buf = kmalloc(T6_USB_XFER_CHUNK_SIZE, GFP_KERNEL);
	if (!chunk_buf)
		return -ENOMEM;
			     ...
			     size_t pixel_len)
{
	...
	video_payload = kvzalloc(total_payload, GFP_KERNEL);
	if (!video_payload)
		return -ENOMEM;
	...
	/*
	 * The working userspace transport sends one protocol header followed by a
	 * continuous video payload stream. t6_bulk_write() still chunks the USB
	 * transfer underneath, but it does not inject additional protocol headers.
	 */
			   ...
			   struct drm_plane_state *old_state)
{
	...
	queue_work(system_long_wq, &t6->tx_work);
}
